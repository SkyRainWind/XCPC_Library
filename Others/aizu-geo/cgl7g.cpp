...
signed main(){
	...
	vector<geo::L>v1 = geo::intanCC(o1,r1,o2,r2);
	vector<geo::L>v2 = geo::extanCC(o1,r1,o2,r2);
//	printf("qwq %d %d\n",v1.size(),v2.size()); 
	vector<geo::P>v;
	for(auto it : v1)v.pb(it[0]);
	for(auto it : v2)v.pb(it[0]);
//	printf("??? %d\n",v.size());
	sort(v.begin(),v.end(),[&](geo::P a,geo::P b){
		if(geo::cmp(a.x, b.x)==0)return a.y < b.y;
		return a.x < b.x;
	});
	cout << fixed << setprecision(9);
	for(int i=0;i<v.size();i++)
//		if(i && (geo::cmp(v[i-1].x,v[i].x) || geo::cmp(v[i-1].y,v[i].y)))
		cout << v[i].x << ' ' << v[i].y << '\n';
