...
signed main(){
	...
	while(m --){
		geo::P q;
		q.read();
		db res = geo::cross(p1,p2,q);
		res = geo::sign(res);
		if(res == 0){
			double dt = segp.dot(q-p1);
			if(geo::sign(dt) < 0)cout << "ONLINE_BACK\n";
			else if(geo::isMiddle(p1,q,p2))cout << "ON_SEGMENT\n";
			else cout << "ONLINE_FRONT\n";
			continue;
		}else if(res < 0)cout << "CLOCKWISE\n";
