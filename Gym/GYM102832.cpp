...
signed main(){
	...
	for(int i=1;i<=n;i++){
		...
		for(int j=1;j<=n;j++){
			if(i == j)continue;
			if((p[i].y>0)!=(p[j].y>0)&&((p[i]-p[j]).det(p[i])>0)==((p[i]-p[j]).det(p[i]-lst))>0){
				while(q--)printf("%.10f\n",lst.x);
				return 0;
			}
			if(p[i].y > 0 && p[j].y > 0 && (lst-p[i]).det(p[j]-p[i]) > 0 && p[j].det(p[i]) > 0)
				d[i] = max(d[i], (p[i]-p[j]).abs() + (p[j]-lst).abs());
	...
	while(q--){
		int x,y;scanf("%d%d",&x,&y);
		if((p[x].y>0) == (p[y].y>0))printf("%.10f\n",lst.x);
		else{
			db ans = p[x].abs()+min((p[x]-p[y]).abs() + (p[y]-lst).abs(), d[x]);
			ans = min(ans, p[y].abs()+min((p[y]-p[x]).abs() + (p[x]-lst).abs(), d[y]));
			printf("%.10f\n",ans);
		}
	}
