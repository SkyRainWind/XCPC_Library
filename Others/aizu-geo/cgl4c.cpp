...
namespace geo{
	...
	db min_dist(vector<P>&ps,int l,int r){
		...
		vector<P> qs; rep(i,l,r) if(fabs(ps[i].x-ps[m].x)<= ret) qs.pb(ps[i]);
		sort(qs.begin(), qs.end(),[&](P a,P b){return a.y==b.y?a.x<b.x:a.y<b.y; });
...
//		if(sign(d) < 0) return {};
//		d = max(d,); P m = p1 - (p2-p1)*(x/y), dr = (p2-p1)*(sqrt(d)/y);
...
signed main(){
	...
	sort(poly.begin(),poly.end(),[&](geo::P a,geo::P b){return a.x==b.x?a.y<b.y:a.x<b.x;});
	db r = geo::min_dist(poly,0,n);
