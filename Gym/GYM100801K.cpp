...
namespace geo{
	...
	P r90c(P v) { return {v.y, -v.x}; }		// ????? 90 
	P getG(P A, P B, P C) {return P{(A.x + B.x + C.x) / 3, (A.y + B.y + C.y) / 3};}	// ???? 
	P getO(P A, P B, P C) { // ?????��?????
...
signed main(){
	...
	for(int i=1;i<=n;i++){
		...
		for(int j=i+1;j<=n;j++){
			db dd = (a[i] - a[j]).abs();
			db phj = (a[j] - a[i]).alpha();
			db theta = solve(phj - ph);
			int ok = 1;
			if(ph != inf && (geo::sign(theta-l) < 0 || geo::sign(theta-r) > 0)){
				ok = 0;
			}
			if(ok == 1)
				gr[i][j] = 1;
