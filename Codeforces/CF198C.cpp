...
int check(db lim){
	db offset = lim * vp / R;
	db target = theta + offset;
//	if(target > pi)target -= 2*pi;
	geo::P now = geo::P(R * cos(target), R * sin(target));
//	cout <<now.x<<' '<<now.y<<'\n';
	
	vector<geo::P> ins = geo::isCL(geo::P(0,0), r, now, go);
//	cout<<ins[0].x<<' '<<ins[0].y<<'\n';
//	cout<<ins[1].x<<' '<<ins[1].y<<'\n';
	if(ins.size() == 0 || (ins[0]-ins[1]).abs() < eps || !geo::isMiddle(now,ins[0],go)){
		db t = straight(now, go);
		if(t-lim < eps)return 1;
		return 0;
	}
//	cout<<ins[0].x<<' '<<ins[0].y<<'\n';
//	cout<<ins[1].x<<' '<<ins[1].y<<'\n';
	vector<geo::P>tan1 = geo::tanCP(geo::P(0,0),r,now);
	vector<geo::P>tan2 = geo::tanCP(geo::P(0,0),r,go);
	db th = fabs(geo::rad(tan1[0], tan2[1]));
	db t1 = (tan1[0]-now).abs()/v + (tan2[1]-go).abs()/v + r*min(th,2*pi-th)/v;
	th = fabs(geo::rad(tan1[1], tan2[0]));
	t1 = min(t1, (tan1[1]-now).abs()/v + (tan2[0]-go).abs()/v + r*min(th,2*pi-th)/v);
//	db t = straight(now, go);
//	db t1 = straight(ins[0],ins[1]);
//	db th = fabs(geo::rad(ins[0],ins[1]));
//	db t2 = r*min(th, 2*pi-th) / v;
//	cout<<"?? "<<t<<' '<<t1 <<' '<<t2<<'\n';
//	cout<<"th="<<th<<'\n';
//	t = t - t1 + t2;
//	cout <<'?' << t<<'\n';
	if(t1-lim < eps)return 1;
	return 0; 
}

signed main(){
	ios::sync_with_stdio(false);
	big.read(); R = big.abs(); cin >> vp;
	go.read(); cin >> v >> r;
	if((big-go).abs() < eps)return cout<<0<<'\n',0;
	
	theta = geo::rad(geo::P(R, 0), big);
//	cout<<check(9.5);return 0;
	db l=0, r = 1e9, ans;
	while(fabs(r-l) > eps){
		db mid = (l+r) / 2;
//		cout<<mid<<'\n';
